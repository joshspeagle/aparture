...
**The hallucination audit fires too often or misses genuine issues.** Edit `prompts/check-briefing.md`. The prompt has explicit YES / MAYBE / NO categories describing what counts as each verdict; adjusting those boundaries shifts sensitivity. Pair with the `briefingRetryOnYes` and `briefingRetryOnMaybe` checkboxes in Settings, which control what the pipeline does after the audit returns.
