...
- **Dry Run** — exercises the full pipeline with mocked LLM responses. Validates UI flow and the pipeline's error-recovery paths. **Cost: $0.**
- **Minimal API Test** — runs 5 fixed test papers through real API calls. Validates that your key authenticates and that the provider responds. **Cost: ~$0.01-0.05** (free on Google's free tier).
...
- **Mock response parsing.** JSON parsing, schema validation, the malformed/missing-field/wrong-type correction loops.
- **Error recovery.** Retry logic, correction prompts, failure escalation.
- **Pause/abort controls.** Stop button and the optional `pauseAfterFilter` / `pauseBeforeBriefing` gates.
...
- Intentional failure scenarios cycle through the test data — expect to see `"Mock parse failed: Response is not an array"` followed by `"Mock correction 1/3 succeeded"` at least once. These aren't real errors; they exercise the retry path.
- A green checkmark on the test card when it finishes.

### If it fails

Dry Run failures are always code or environment issues — never API-related (no real API calls happen). Common causes:
...
The Minimal Test button unlocks only after Dry Run completes successfully. This enforcement prevents running up real API spend against a broken UI.

### What it validates

- **API key authentication.** A bad key surfaces as `401 - Unauthorized` immediately.
- **Billing + quota.** Insufficient credits or hit quotas return `402` or provider-specific errors.
- **Provider latency + rate limits.** Real round-trip times; provider rate-limit backoff if hit.
...
- **Paper fetch timing** (skipped; fixture is loaded directly).

### What to expect

Duration: 30-120 seconds (dominated by API latency + model processing).
...
Cache-hit metrics (Anthropic/OpenAI) show as `[anthropic cache] read=N create=N` lines. Read > 0 on call 2+ means caching is working.
...
| `PERMISSION_DENIED` (Google)                                            | Model gated (e.g., Gemini 3.1 Pro on free tier) | Either enable billing or switch to Budget preset                                                                                    |
...
- Dry Run card shows a green check and `"Dry run test completed successfully"`.
- Minimal Test card shows a green check, a last-run timestamp, and `"Minimal test completed successfully"`.
...
- The terminal log shows a clean sequence of `Sending request to ...` lines per stage, no red errors, no repeated `API error 429` or validation-failed messages.
