...
This is the most important thing in the dialog. It lets you see exactly what your profile will look like before you commit. If the preview looks weird — say, a crucial paragraph has disappeared or an anti-interest line reads oddly — you can toggle cards until it looks right.
...
Reverting is safe — your current profile is also kept in history, so you can always flip back.

There's also a **Clear history** button if your history has gotten long and unwieldy. It wipes all revisions but keeps the current profile intact.
...
Don't run it every day. The system works best when feedback accumulates over several briefings — the LLM needs volume and variety to spot patterns. Running it after a single briefing usually produces "no clear change" or overfits to one day's papers.
...
- Would a collaborator reading this profile recognize your research?
...
- **Know about papers it hasn't seen.** The LLM only reads your starred/dismissed papers + comments + overrides. If you star a paper outside Aparture (e.g. on arXiv itself), that signal doesn't reach Suggest Improvements.
...
- You've refined your profile and want to tune the actual pipeline — models, thresholds, batch sizes. → [Tuning the pipeline](/using/tuning-the-pipeline)
