...
function detectHallucination(generatedText, originalPapers) {
  ...
  while ((match = quoteRegex.exec(generatedText)) !== null) {
    ...
    const _contextAfter = generatedText
      .substring(
        position + match[0].length,
        Math.min(generatedText.length, position + match[0].length + 50)
      )
      .trim();
    ...
    const likelyTitle =
      words.filter((w) => w[0] && w[0] === w[0].toUpperCase()).length > words.length * 0.5;
    if (likelyTitle) {
  ...
  for (const potentialTitleObj of quotedStrings) {
    ...
    if (!found && closestMatch) {
      ...
      const maxSimilarity =
        1 - closestDistance / Math.max(closestMatch.length, potentialTitle.length);
  ...
  const _pRefs = generatedText.match(pRefRegex) || [];
  ...
  while ((refMatch = refWithTitle.exec(generatedText)) !== null) {
    ...
    if (paperId <= originalPapers.length) {
      ...
      if (actualTitle !== claimedTitle) {
        const similarity =
          1 -
          levenshteinDistance(actualTitle.toLowerCase(), claimedTitle.toLowerCase()) /
            Math.max(actualTitle.length, claimedTitle.length);

        if (similarity < 0.9) {
          issues.push(
            `Wrong title for [P${paperId}]: got "${claimedTitle}", expected "${actualTitle}"`
          );
          detailedIssues.push({
...
async function callClaude(modelName, prompt) {
  ...
  console.log('Sending NotebookLM generation request to Anthropic:', {
    model: modelName,
...
async function callOpenAI(modelName, prompt) {
  ...
  console.log('Sending NotebookLM generation request to OpenAI:', {
    model: modelName,
    promptLength: prompt.length,
  });
...
async function callGemini(modelName, prompt) {
  ...
  console.log('Sending NotebookLM generation request to Google AI:', {
    model: modelName,
    promptLength: prompt.length,
  });
...
${relevantPapers
  .map(
    (p, idx) => `[P${idx + 1}] Title: "${p.title}"
    Score: ${p.score || p.relevanceScore}/10${p.adjustedScore ? ` (Adjusted: ${p.adjustedScore}/10)` : ''}`
  )
  .join('\n')}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  try {
    ...
    if (enableHallucinationCheck) {
      ...
      while (check.hasHallucination && correctionAttempts < maxCorrectionRetries) {
        ...
        console.log(
          `HALLUCINATION DETECTED (Attempt ${correctionAttempts + 1}/${maxCorrectionRetries}):`,
      ...
      if (check.hasHallucination && correctionAttempts === maxCorrectionRetries) {
        ...
        if (finalCheck.hasHallucination) {
          console.error(
            'WARNING: Some hallucination issues persist even after strict mode:',
            finalCheck.issues
          );
          // Add persistent issues to warnings
          warnings = [
            ...warnings,
            ...finalCheck.issues.map((i) => `[Persistent after strict mode] ${i}`),
          ];
        } else {
    ...
    if (finalHallucinationCheck) {
      metadata.finalStatus = {
        ...
        correctionMethod: useStrictMode
          ? 'strict_mode'
