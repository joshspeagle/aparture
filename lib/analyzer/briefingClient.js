...
// expansions.
async function generateQuickSummaries({ papers, provider, modelId, password, concurrency }) {
  const quickById = {};
  const papersWithReports = papers.filter((p) => p.fullReport);
  // Clamp to a sane range so a corrupted config can't stall the run or
  // drown the provider in simultaneous requests.
  const chunkSize = Math.max(1, Math.min(20, concurrency ?? 5));
  for (let i = 0; i < papersWithReports.length; i += chunkSize) {
    const chunk = papersWithReports.slice(i, i + chunkSize);
    await Promise.all(
      chunk.map(async (p) => {
        try {
