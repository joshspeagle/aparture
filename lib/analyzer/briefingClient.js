...
export async function runBriefingGeneration({
  ...
  try {
    ...
    const synthJson = mockTester
      ? await mockTester.mockSynthesize(papers)
    ...
    let finalCheck = null;
    let retried = false;
    if (!mockTester) onUsage('briefing', modelId, synthJson);

    // Hallucination check + optional retry
    throwIfAborted();
    setBriefingStage('checking');
    finalCheck = mockTester
      ? await mockTester.mockCheckBriefing()
      : await callCheckBriefing({
    ...
    // The check route returns tokensIn/tokensOut too; both synthesis and the
    // check run on the same model, so they accumulate into one briefing stage.
    if (!mockTester && finalCheck) onUsage('briefing', modelId, finalCheck);
    ...
    if (finalCheck) {
      ...
      if (shouldRetry) {
        ...
        try {
          ...
          // is unreachable on a dry run today, but keep the mock guard so
          // the "mockTester means zero fetches" invariant survives future
          // changes to the mock verdict.
          const retryJson = mockTester
            ? await mockTester.mockSynthesize(papers)
            : await callSynthesize({
                profile: profile.content,
                papers,
          ...
          if (!mockTester) onUsage('briefing', modelId, retryJson);
          // Re-run the check on the retry so the badge reflects the final state
          setBriefingStage('checking');
          const recheck = mockTester
            ? await mockTester.mockCheckBriefing()
            : await callCheckBriefing({
                briefingObj: finalBriefing,
                papers,
          ...
          if (recheck) {
            if (!mockTester) onUsage('briefing', modelId, recheck);
            finalCheck = recheck;
          }
          retried = true;
        } catch (retryErr) {
