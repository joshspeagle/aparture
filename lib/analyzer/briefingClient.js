...
export async function runBriefingGeneration({
  ...
  try {
    ...
    // Phase B-prep: forward generationMetadata (may be null for callers
    // that don't supply it). useBriefing.saveBriefing omits the field
    // from the persisted entry when undefined.
    saveBriefing(today, finalBriefing, generationMetadata ?? undefined);
