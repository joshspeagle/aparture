...
export async function generateQuickSummaries({
  ...
  abortSignal,
  addStatus = () => {},
  // Optional (entry) callback for recording a safety-classifier refusal into
  // the run's refusals slice. No-op default, same as addStatus/onUsage.
  onRefusal = () => {},
  ...
  await pool.run(papersWithReports, async (p) => {
    try {
      ...
      try {
        ...
        // A refusal is deterministic — the manual retry below would ask the
        // same classifier the same question and get the same no, billing the
        // input tokens again. Record and re-throw to the non-fatal catch.
        if (isContentRefusal(err)) {
          onRefusal({
            stage: 'quickSummary',
            scope: p.title ?? p.arxivId,
...
export async function runBriefingGeneration({
  ...
  setFullReportsById,
  addStatus = () => {},
  // Optional (entry) callback used to record a safety-classifier refusal
  // into the run's refusals slice. Same no-op-default convention as
  // addStatus/onUsage so existing call sites and tests keep working.
  onRefusal = () => {},
  ...
  try {
    ...
    // A safety-classifier refusal can't be skipped here the way a single
    // paper can — synthesis is one call for the whole briefing, so there is
    // no partial result to fall back to. Say plainly what happened and what
    // to change, instead of surfacing a bare provider string.
    if (isContentRefusal(err)) {
      onRefusal({
        stage: 'briefing',
        scope: 'briefing synthesis',
        provider: err.provider,
        model: err.model,
        category: err.category,
        raw: err.raw,
      ...
      setSynthesisError(
        `${err.provider ?? 'The provider'} declined to generate the briefing (${err.category ?? 'unknown'}). ` +
          `The papers and their analyses are unaffected — only the synthesis step failed. ` +
          `Try a different briefingModel, or set refusalPolicy to 'fallback' with a refusalFallbackModel.`
      );
    } else {
