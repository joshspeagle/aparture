...
export class CacheWarmupGate {
  ...
  async run(provider, fn, abortSignal = null) {
    ...
    if (!existing.open) {
      ...
      // The wait is the only place time passes inside the gate, so it's the
      // only place an abort can land mid-flight. raceAbort resolves on abort
      // as well as on the warmer settling, so without this check a follower
      // would go on to issue a real, billable request after the user hit
      // Stop. Throw the pipeline's standard abort shape — makeRobustAPICall
      // re-throws it unwrapped.
...
function raceAbort(promise, abortSignal) {
  ...
  return new Promise((resolve) => {
    ...
    const finish = () => {
      if (settled) return;
      settled = true;
      // Detach explicitly: `once` only self-removes when abort actually
      // fires, so on the far more common path (the warmer settles first) the
      // handler would stay attached for the life of the signal. One signal
      // spans a whole run, so a run with many gated calls would accumulate
      // handlers and trip Node's MaxListeners warning.
