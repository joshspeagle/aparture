...
*Document prepared for NotebookLM podcast generation. Target duration: ${targetDuration} minutes. Model: ${model}. Generated at: ${new Date().toISOString()}*`;
  ...
  async mockQuickSummary(paper) {
    await this.checkAbortAndPause();
    this.callCount++;
    await this.sleepWithAbortCheck(50 + Math.random() * 100);
    console.log(`Mock Quick Summary API Call ${this.callCount}: Summarizing "${paper.title}"`);
    await this.checkAbortAndPause();
    return (
      `Mock quick summary of "${paper.title}" generated for dry-run testing. ` +
      `This placeholder stands in for the real compression of the full report. ` +
      `No LLM call was made and no analysis was performed.`
    );
  }

  async mockSynthesize(papers) {
    await this.checkAbortAndPause();
    this.callCount++;
    await this.sleepWithAbortCheck(150 + Math.random() * 150);
    console.log(
      `Mock Synthesize API Call ${this.callCount}: Building briefing from ${papers.length} papers`
    );
    await this.checkAbortAndPause();

    const clampScore = (value) =>
      typeof value === 'number' ? Math.max(0, Math.min(10, value)) : 5;
    const ids = papers.map((p) => p.arxivId);
    const mid = Math.max(1, Math.ceil(ids.length / 2));
    const themes = [
      {
        title: 'Mock Theme 1: Simulated Cross-Paper Connections',
        argument:
          'Mock theme synthesized for testing. This argument is placeholder prose that cites real papers from the run without performing any actual synthesis.',
        paperIds: ids.slice(0, mid),
      },
    ];
    if (ids.length > mid) {
      themes.push({
        title: 'Mock Theme 2: Placeholder Methodological Trends',
        argument:
          'Mock theme synthesized for testing. A second placeholder grouping so the dry-run briefing renders multiple theme sections.',
        paperIds: ids.slice(mid),
      });
    }

    // Same shape as the real /api/synthesize 200 response consumed by
    // briefingClient. The briefing object passes lib/synthesis/validator.js
    // (zod schema + citation validation) because every arxivId is taken
    // verbatim from the input papers.
    return {
      briefing: {
        executiveSummary:
          `Mock executive summary generated by the dry-run tester across ${papers.length} papers. ` +
          'This briefing is gibberish by design: it validates rendering, persistence, and the ' +
          'TEST MODE flow without making any LLM calls.',
        themes,
        papers: papers.map((p) => ({
          arxivId: p.arxivId,
          title: p.title,
          score: clampScore(p.score),
          onelinePitch: `Mock one-line pitch for "${p.title}".`,
          whyMatters: `Mock relevance rationale for "${p.title}", synthesized for testing only.`,
        })),
      },
      tokensIn: 0,
      tokensOut: 0,
  ...
  async mockCheckBriefing() {
    await this.checkAbortAndPause();
    this.callCount++;
    await this.sleepWithAbortCheck(50 + Math.random() * 100);
    console.log(`Mock Check Briefing API Call ${this.callCount}: Returning passing verdict`);
    await this.checkAbortAndPause();
    // Same shape as the real /api/check-briefing 200 response. Verdict NO
    // means "no hallucinations found" — a pass — so the dry run never takes
    // the retry-synthesis path.
    return {
      verdict: 'NO',
      justification:
        'Mock hallucination check for dry-run testing. No real audit was performed; this verdict is a simulated pass.',
      flaggedClaims: [],
      tokensIn: 0,
      tokensOut: 0,
      repaired: false,
