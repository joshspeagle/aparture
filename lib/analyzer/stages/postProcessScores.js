...
export function createPostProcessScores(deps) {
  ...
  const postProcessScores = async (papers, isDryRun = false) => {
    ...
    // Stage 3.5 runs on its own slot. `postProcessingModel` had been
    // defined in DEFAULT_CONFIG and documented as the slot driving this
    // stage, but the request body was sending `config.scoringModel`, so
    // setting it had no effect. Fall back to scoringModel for legacy
    // configs saved before the slot existed; DEFAULT_CONFIG sets both to
    // the same model, so this is a no-op for stock installs.
    const postProcessingModel = config.postProcessingModel ?? config.scoringModel;
    // Provider lookup must track the model actually dispatched, so cache
    // warmup and the rate-limit barrier land on the right provider.
    const providerLower = providerKeyForModel(postProcessingModel);
      ...
      try {
        ...
        if (isDryRun) {
          ...
          const makeAPICall = async (
            ...
            const effectiveModel = modelOverride ?? postProcessingModel;
            const requestBody = {
              papers: batch.map((p) => ({
                title: p.title,
                abstract: p.abstract,
            ...
            recordUsage('postProcessing', effectiveModel, data);
        ...
        // Safety-classifier refusal: record it so the run summary can
        // explain the gap, then fall through to this stage's existing
        // degrade-and-continue path. Under refusalPolicy 'fail',
        // makeRobustAPICall marks the error fatal and we re-throw instead.
        if (isContentRefusal(error)) {
          store().addRefusal({
            stage: 'postProcessing',
            scope: `batch ${batchIndex + 1} (${batch.length} papers)`,
            provider: error.provider,
            model: error.model,
            category: error.category,
            raw: error.raw,
          });
          if (error.refusalFatal) throw error;
        }
