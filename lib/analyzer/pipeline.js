...
export function createAnalysisPipeline({ abortControllerRef, pauseRef, mockAPITesterRef }) {
  ...
  // Skip-remaining-gates flag (wired in Phase 7). Optional-chaining throughout
  // the pipeline so code compiles and runs correctly today.
  ...
  const startProcessing = async (isDryRun = false, useTestPapers = false) => {
    ...
    try {
      ...
      // Resolve the additive PDF set after the gate
      const { resolveAdditiveSet } = await import('./resolveAdditiveSet.js');
      const freshStore = store();
      const topPapers = resolveAdditiveSet({
        availablePapers,
        maxDeepAnalysis: config.maxDeepAnalysis,
      ...
      const topByScore = analyzedPapers.slice(0, config.finalOutputCount);
