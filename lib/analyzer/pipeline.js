      ...
      try {
        ...
        if (isDryRun) {
          ...
          const makeAPICall = async (correctionPrompt = null, isCorrection = false) => {
            ...
            if (!response.ok) {
              const errorData = await response.json();
              // Structured Playwright-unavailable response: propagate as a
              // sentinel error so the outer catch can aggregate this paper
              // into `skippedDueToRecaptcha` without logging a hard failure.
              if (
                response.status === 422 &&
        ...
        // the run. `makeRobustAPICall` wraps the sentinel error in its own
        // "All retries and corrections exhausted" message after maxRetries,
        // so detect the code via either the direct error or its cause.
        ...
        if (isPlaywrightSkip) {
          ...
          analyzedPapers.push(skippedPaper);

          // Keep the finalRanking in sync so downstream UI sees the marker
          setProcessing((prev) => ({
            ...prev,
            progress: { current: i + 1, total: papers.length },
          }));
