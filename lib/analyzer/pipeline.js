...
function providerKeyForModel(modelId) {
...
export function createAnalysisPipeline({ abortControllerRef, pauseRef, mockAPITesterRef }) {
  ...
  const scoreAbstracts = async (papers, isDryRun = false) => {
    ...
    try {
      ...
      const today = runBriefingDateRef.current ?? new Date().toISOString().slice(0, 10);
  ...
  const startProcessing = async (isDryRun = false, useTestPapers = false) => {
    ...
    // Full results reset (replacement, not merge): drops run-added keys
    // (availablePapers, failedPapers, allAnalyzedPapers) left behind by a
    // previous run — setResults' merge semantics would otherwise let stale
    // entries leak into this run's UI and scoredPaperIds labeling.
    resetResults();
    ...
    // metadata use a consistent date even on long overnight runs.
    runBriefingDateRef.current = new Date().toISOString().slice(0, 10);
    ...
    try {
      ...
      if (finalPapers.length > 0) {
        ...
        const resolvedBriefingModel =
          latestConfig?.briefingModel ?? latestConfig?.pdfModel ?? DEFAULT_MODEL_ID;
