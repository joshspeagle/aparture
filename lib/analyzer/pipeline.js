...
//
// createAnalysisPipeline(stateRef) returns an object of handlers closed over
// a single mutable state ref. ArxivAnalyzer updates stateRef.current on every
// render so each handler reads current values at call time — no stale closure
// capture bugs. React state updates happen via injected setters; this module
// has no React imports and is unit-testable with a fake stateRef.

import { MockAPITester } from './mockApi.js';
import { TEST_PAPERS } from '../../utils/testUtils.js';

export function createAnalysisPipeline(stateRef) {
  const waitForResume = () => {
    const { pauseRef } = stateRef.current;
  ...
  const makeRobustAPICall = async (
    ...
    const { abortControllerRef, addError, config, pauseRef } = stateRef.current;
    let lastError = null;
  ...
  const executeArxivQuery = async (query, maxResults, category) => {
    const { abortControllerRef, password } = stateRef.current;
  ...
  const fetchSingleCategory = async (category, statusCallback) => {
    const { abortControllerRef, pauseRef } = stateRef.current;
  ...
  const fetchPapers = async () => {
    const { abortControllerRef, addError, config, pauseRef, setProcessing, setResults } =
      stateRef.current;
  ...
  const performQuickFilter = async (papers, isDryRun = false) => {
    ...
    // Compute final counts from allVerdicts (the local accumulator). Reading
    // stateRef.current.filterResults would return the destructured snapshot
    // taken at call time (all zeros), since React state updates during the
    // loop haven't landed on the captured object.
  ...
  const runDryRunTest = async () => {
    const { abortControllerRef, addError, mockAPITesterRef, pauseRef, setTestState } =
      stateRef.current;
  ...
  const runMinimalTest = async () => {
    const { abortControllerRef, addError, setTestState } = stateRef.current;
