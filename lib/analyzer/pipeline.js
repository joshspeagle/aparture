    ...
    await filterPool.run(batches, async (batch, batchIndex) => {
      ...
      try {
        ...
        batch.forEach((paper) => {
          ...
          // Push into the MAYBE bucket like the success path does — the
          // filter-review gate rebuilds papersToScore exclusively from the
          // buckets, so papers absent here would vanish from the run (and
          // from the FilterResultsList UI).
          setFilterResults((prev) => ({ ...prev, maybe: [...prev.maybe, paper] }));
