...
export function createAnalysisPipeline({ abortControllerRef, pauseRef, mockAPITesterRef }) {
  ...
  // arXiv asks for >= 3s between requests. Jitter to 3000–5000ms so
  // many back-to-back queries don't look like a metronome.
  const arxivSpacingMs = () => 3000 + Math.floor(Math.random() * 2000);
  ...
  const executeArxivQuery = async (query, maxResults, category) => {
    ...
    // arXiv recommends >= 3s between requests. On 429 or 5xx, back off
    // exponentially (5s → 15s → 45s) rather than shifting the date range,
    // since the rate limit is endpoint-wide and date-shifting doesn't help.
    ...
    for (let attempt = 0; attempt <= maxRateLimitRetries; attempt++) {
      ...
      // The server proxy collapses arXiv 5xx into 429-with-upstreamStatus so
      // the retry path here handles both identically.
      if (response.status === 429) {
        ...
        const upstream = errorData.upstreamStatus || 429;
        const backoffMs = retryAfterSec
          ? retryAfterSec * 1000
          : baseBackoffMs * Math.pow(3, attempt);
        console.warn(
          `  arXiv ${upstream}, backing off ${Math.round(backoffMs / 1000)}s (attempt ${attempt + 1}/${maxRateLimitRetries})`
        );
  ...
  const fetchSingleCategory = async (category, statusCallback) => {
    ...
    for (let i = 0; i < dayShiftSchedule.length; i++) {
      ...
      try {
        ...
        if (!isLastAttempt) {
          await new Promise((resolve) => setTimeout(resolve, arxivSpacingMs()));
        }
      } catch (error) {
