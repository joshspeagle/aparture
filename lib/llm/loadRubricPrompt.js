 ...
 * @param {string} [baseDir] - directory containing the prompt files. Defaults to the
 *                             live `prompts/` dir; tests inject a temp dir so scratch
 *                             templates never land where prompts go live without rebuild.
...
export async function loadRubricPrompt(
  ...
  stableVars = {},
  variableVars = {},
  baseDir = path.resolve(process.cwd(), 'prompts')
) {
  const templatePath = path.resolve(baseDir, filename);
...
/**
 * Build the self-contained body for a validation-failure RETRY call.
 *
 * The retry is ALWAYS issued uncached (`cacheable: false`), and the
 * Anthropic adapter ignores `cachePrefix` whenever `cacheable === false`.
 * So the body must be fully self-contained: it must always inline the
 * cache-stable prefix (rubric + research profile) ahead of the variable
 * tail (per-batch papers). Dropping the prefix on retry would strip the
 * rubric + profile from the very call that follows a validation failure —
 * exactly when full context matters most.
 *
 * When an `APARTURE_TEST_PROMPT_OVERRIDE` is active, it fully replaces the
 * body so fixture hashes stay deterministic (matching the first-attempt
 * `finalPrompt` shape in every rubric route).
 *
 * @param {object} args
 * @param {string|undefined} args.promptOverride - APARTURE_TEST_PROMPT_OVERRIDE, if set
 * @param {string} args.cachePrefix - cache-stable rubric + profile prefix
 * @param {string} args.variableTail - per-batch variable tail (papers section)
 * @returns {string} the uncached, self-contained retry body (no error hint)
 */
export function buildRetryPrompt({ promptOverride, cachePrefix, variableTail }) {
  return promptOverride ?? cachePrefix + variableTail;
}
...
function substitute(template, vars) {
  ...
  for (const [key, value] of Object.entries(vars)) {
    ...
    // Function replacement: a string value would get GetSubstitution
