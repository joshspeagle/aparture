...
// Google finishReason values that mean "the model declined on content
// grounds", as distinct from ones that mean "generation stopped for a
// mechanical reason". MAX_TOKENS is deliberately absent: a truncated
// response is a budget problem that a retry or a larger maxTokens can fix,
// so it must stay on the generic-error path rather than being reported to
// the user as a safety refusal.
const GOOGLE_REFUSAL_FINISH_REASONS = new Set([
  'SAFETY',
  'PROHIBITED_CONTENT',
  'BLOCKLIST',
  'SPII',
...
export function parseGoogleResponse(response, options = {}) {
  ...
  // Content refusals are checked before (and independently of)
  // expectStructured: a safety block is a refusal whether or not the caller
  // wanted JSON back, and it must be typed so the retry ladder skips it —
  // re-asking a classifier that already said no just burns quota.
  ...
  if (finishReason && GOOGLE_REFUSAL_FINISH_REASONS.has(finishReason)) {
    throw new RefusalError({
      provider: 'google',
      model: response.modelVersion ?? null,
      raw: finishReason,
      message: `google declined this request (finishReason: ${finishReason})`,
    });
  }

  if (options.expectStructured) {
    // Surface the real cause when Google returns no usable text — a
    // truncated or empty candidate would otherwise show up as an opaque
    // "Unexpected end of JSON input". (Safety blocks were already handled
    // above as typed refusals.)
    if (!text) {
      if (!first || (finishReason && finishReason !== 'STOP')) {
        throw new Error(
          `Google returned no structured output (finishReason: ${finishReason ?? 'no candidates'})`
        );
      }
    }
    try {
