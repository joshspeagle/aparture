...
import { RefusalError } from '../RefusalError.js';
...
export function parseOpenAIResponse(response, options = {}) {
  ...
  // Chat Completions signals a decline two ways: `finish_reason:
  // "content_filter"` (the moderation layer cut the response) and a non-null
  // `message.refusal` string (the model itself declined, which is what
  // structured-output requests get). Either is a refusal.
  const refusalText = choices[0]?.message?.refusal;
  const finishReason = choices[0]?.finish_reason;
  if (finishReason === 'content_filter' || (typeof refusalText === 'string' && refusalText)) {
    throw new RefusalError({
      provider: 'openai',
      model: response.model ?? null,
      raw: finishReason === 'content_filter' ? 'content_filter' : 'refusal',
      explanation: typeof refusalText === 'string' ? refusalText : null,
    });
  }
 ...
 *
 * Structured output in the Responses API uses `text.format.json_schema` (NOT
 * `response_format` like Chat Completions). Same strict-mode constraints apply:
 * the root schema must be an object (top-level arrays rejected).
 *
 * @param {Object} input
 * @param {string} input.model              OpenAI model id (e.g. 'gpt-5.4')
 ...
 * @param {Object} [input.structuredOutput] Optional schema for forced JSON output
 * @param {string}   input.structuredOutput.name    Schema name
 * @param {Object}   input.structuredOutput.schema  JSON schema (root must be object)
 * @returns {{ url: string, method: 'POST', headers: Object, body: Object }}
 */
export function buildOpenAIResponsesRequest({ model, prompt, pdfBase64, structuredOutput }) {
  const body = {
    model,
    input: [
      {
 ...
 *
 * The Responses endpoint returns an `output` array of output items. Each item
 * with `type === 'message'` contains a `content` array; the text lives at
 * `output[0].content[0].text`. Token counts are under `usage.input_tokens`
 ...
 * `structured` (the Responses API returns structured output as JSON text when
 * text.format.json_schema is set, same as Chat Completions).
 *
 ...
 * @returns {{ text: string, tokensIn: number, tokensOut: number, structured?: any }}
 */
export function parseOpenAIResponsesResponse(response, options = {}) {
  const outputItems = response.output ?? [];
  const messageItem = outputItems.find((item) => item.type === 'message') ?? outputItems[0];
  // The Responses API expresses a decline as a `refusal` content block on
  // the message item, or as an incomplete response with reason
  // "content_filter". Shape differs from Chat Completions, so it needs its
  // own check rather than sharing the one above.
  const refusalBlock = messageItem?.content?.find((c) => c?.type === 'refusal');
  const incompleteReason = response.incomplete_details?.reason;
  if (refusalBlock || incompleteReason === 'content_filter') {
    throw new RefusalError({
      provider: 'openai',
      model: response.model ?? null,
      raw: refusalBlock ? 'refusal' : 'content_filter',
      explanation: refusalBlock?.refusal ?? null,
    });
  }
  ...
  if (options.expectStructured) {
    try {
      ...
      throw new Error(`failed to parse OpenAI Responses structured output as JSON: ${err.message}`);
    }
  }
  return out;
}
