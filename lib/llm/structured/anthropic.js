...
import { supportsAdaptiveThinkingByApiId } from '../../../utils/models.js';

// Adaptive thinking (`thinking: {type: 'adaptive'}`) exists only on Claude
// Opus/Sonnet 4.6 and later (including the 5.x line). The registry
// (utils/models.js) is the single source of truth: every Anthropic
// MODEL_REGISTRY entry carries a `supportsAdaptiveThinking` flag, consulted
// first via supportsAdaptiveThinkingByApiId. The version-parsing fallback
// below applies ONLY to apiIds absent from the registry (smoke-script
// `--model-anthropic=<id>` overrides, unregistered/raw ids), so every model
// the adapter might see gets a request shape its API accepts.
function modelSupportsThinking(model) {
  if (!model) return false;
  const registered = supportsAdaptiveThinkingByApiId(model);
  if (registered !== undefined) return registered;
  // Regex fallback for unregistered ids. Adaptive is NOT supported on Haiku
  // (verified via live 400: "adaptive thinking is not supported on this
  // model" against claude-haiku-4-5) — haiku ids don't match the family
  // group and fall through to false. Pre-4.6 Opus/Sonnet models
  // (e.g. claude-opus-4-5, claude-sonnet-4-5, claude-opus-4-1) require the
  // older `{type: 'enabled', budget_tokens}` shape and reject adaptive, so
  // the version gate excludes them too.
  // Matches apiIds like claude-opus-4-8, claude-sonnet-5, claude-sonnet-4-6,
  // and dotted user-facing forms like claude-opus-4.7. Date-stamped suffixes
  // (claude-sonnet-4-5-20250929) parse as major 4 / minor 5 as intended.
  // Older date-first ids (claude-3-5-sonnet-20241022) don't match and
  // correctly fall through to false.
  const match = model.toLowerCase().match(/^claude-(?:opus|sonnet)-(\d+)(?:[-.](\d+))?/);
  if (!match) return false;
  const major = parseInt(match[1], 10);
  // A "minor" with 4+ digits is a date stamp directly after the major
  // (claude-opus-4-20250514), not a real minor version — treat as .0.
...
export function parseAnthropicResponse(response) {
  ...
  // empty content array (pre-output) or a partial one (mid-stream). Check
  // before reading content so a refusal surfaces as a typed RefusalError
  // rather than an empty-structured-output parse failure downstream.
  // `stop_details` is informational and can be null even on a refusal, so
  // branch on stop_reason and treat stop_details as best-effort detail.
  if (response.stop_reason === 'refusal') {
    throw new RefusalError({
      provider: 'anthropic',
      model: response.model ?? null,
      raw: response.stop_details?.category ?? null,
