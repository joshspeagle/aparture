...
export async function callModel(input, options = { mode: 'live' }) {
  ...
  // Spread input into effectiveInput so we can safely forward it to adapters.
  // (pdfBase64 is forwarded as-is in live mode; the caller is responsible for
  // any test isolation of that field before calling callModel.)
