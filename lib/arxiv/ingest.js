...
async function harvestSubcategoryViaAtom({
  ...
  statusCallback,
  fetchAtomImpl,
  // Anchor-fallback re-fetch (see harvest below): when the un-widened
  // fast-path window came back with zero v1 days, the caller retries once
  // with the full widened window so the anchor can land on an older
  // announcement day instead of defaulting to window.until.
  forceWidenedWindow = false,
}) {
  // Atom's `submittedDate:[…]` query filters on v1 server-side, so the
  // announce-lag widening the OAI path needs (window.from is widened
  // backward by ANNOUNCE_LAG_BUFFER_DAYS in pipeline.fetchPapers) buys
  // nothing here — it only burns the single unpaginated 300-result budget
  // on extra days the user didn't ask for, which for cs.LG-scale
  // categories overflows the cap and silently truncates the target day
  // (and can skew the v1 anchor). In submitted-only mode with a
  // production targetDaysBack, fetch the UN-widened v1 target window
  // instead. submitted-or-updated (never widened) and the legacy/test
  // path without targetDaysBack keep the window as-is.
  const useTargetWindow =
    !forceWidenedWindow &&
    window.windowSemantics === 'submitted-only' &&
    typeof window.targetDaysBack === 'number' &&
    window.targetDaysBack >= 1;
  const atomFrom = useTargetWindow
    ? shiftIsoDate(window.until, -(window.targetDaysBack - 1))
    : window.from;

  // Cache broad Atom fetches under the same TTL as the OAI-prefix path —
  // previously only OAI (and fill-ups) were cached, so an atom-only user
  // re-running within the TTL re-downloaded every subcategory. Keyed on the
  // EFFECTIVE fetch window (atomFrom, which folds windowSemantics /
  // targetDaysBack into the dates), so no semantics component is needed in
  // the key: these are raw pre-filter records, like the OAI broad entries.
  const ttlMinutes = window.cacheTtlMinutes ?? 0;
  if (ttlMinutes > 0) {
    const hit = cacheLookup({
      set: subcategory,
    ...
    if (hit) {
      const ageMin = Math.round((Date.now() - hit.cachedAt) / 60_000);
      statusCallback(`${subcategory} (cached, ${ageMin} min ago)`);
      return {
        records: hit.records,
        perPrefixEntry: {
          prefix: subcategory,
...
export async function harvest(window, deps) {
  ...
  // Subcategories fetched via the broad Atom path (atom-only mode or the
  // auto-mode breaker fallback). Those fetches use the UN-widened target
  // window in submitted-only mode, so when the whole broad fetch yields zero
  // v1 days the anchor fallback below re-fetches exactly these with the
  // widened window before computing the anchor.
  const atomBroadSubcats = [];
  ...
  if (window.mode === 'atom-only') {
    ...
    for (let i = 0; i < total; i++) {
      ...
      atomBroadSubcats.push(subcategory);
    ...
    for (let i = 0; i < prefixes.length; i++) {
      ...
      if (useOai) {
        try {
          ...
          // An abort mid-OAI-fetch is not an OAI failure — re-throw rather
          // than tripping the breaker and emitting a misleading fallback status.
          if (abortSignal?.aborted || err?.message === 'Operation aborted') throw err;
  ...
  const applySelection = (records) =>
    window.mode === 'atom-only' ? records : filterToSelected(records, window.selectedSubcategories);
  let filtered = applySelection(allRecords);
  ...
  // Anchor fallback for the un-widened Atom fast path. In submitted-only mode
  // the broad Atom fetch deliberately asks for only the un-widened v1 target
  // window (see harvestSubcategoryViaAtom). On a day with no announcements in
  // that window (e.g. Monday with daysBack=1 — nothing announced over the
  // weekend), that fetch returns ZERO records, the anchor below would default
  // to window.until, and the run degrades into fill-up grab-bags. Re-fetch
  // ONCE with the full widened window so the anchor can land on the most
  // recent announcement day with content.
  if (anchorMode && atomBroadSubcats.length > 0) {
    const unwidenedFrom = shiftIsoDate(window.until, -(window.targetDaysBack - 1));
    const hasV1Day = filtered.some((p) => (p.published ?? '').slice(0, 10));
    if (!hasV1Day && window.from < unwidenedFrom) {
      statusCallback(
        `No v1 submissions in [${unwidenedFrom}, ${window.until}] — widening Atom lookback to ${window.from}`
      );
      for (const subcategory of atomBroadSubcats) {
        if (abortSignal?.aborted) throw new Error('Operation aborted');
        if (waitForResume) await waitForResume();
        await spaceIfNotFirst();
        statusCallback(`Fetching ${subcategory} (widened)…`);
        const { records, perPrefixEntry } = await harvestSubcategoryViaAtom({
          subcategory,
          window,
          password,
          ...
          statusCallback,
          fetchAtomImpl,
          forceWidenedWindow: true,
        });
        // Replace (not append) the first pass's entry and records for this
        // subcategory — the widened fetch supersedes the empty un-widened one.
        const priorIdx = perPrefix.findIndex((e) => e.prefix === subcategory);
        if (priorIdx >= 0) perPrefix[priorIdx] = perPrefixEntry;
        else perPrefix.push(perPrefixEntry);
        for (let j = allRecords.length - 1; j >= 0; j--) {
          if (allRecords[j].fetchedCategory === subcategory) allRecords.splice(j, 1);
        }
        allRecords.push(...records);
        statusCallback(`✓ ${subcategory}: ${records.length} papers (widened)`);
      }
      filtered = applySelection(allRecords);
    }
  }
  ...
  const fillupFetchFn = async ({ narrowSet, from, until, subcategory }) => {
    ...
    if (ttlMinutes > 0) {
      // windowSemantics is part of the fill-up cache key: these entries hold
      // post-filtered records, so toggling the semantics within TTL must not
      // serve records filtered under the other setting.
      const hit = cacheLookup({
        set: cacheKeySet,
        from,
        until,
        mode: window.mode,
        windowSemantics: window.windowSemantics,
        ttlMinutes,
      });
