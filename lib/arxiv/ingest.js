...
export async function harvest(window, deps) {
  ...
  // applied before the first request of a loop — only between them — so a
  // single-subcategory fetch isn't needlessly delayed. See lib/arxiv/spacing.js.
  const spaceBetweenRequests = () => sleepImpl(spacingMsImpl());
  ...
  if (window.mode === 'atom-only') {
    ...
    for (let i = 0; i < total; i++) {
      ...
      if (i > 0) await spaceBetweenRequests();
    ...
    // track "has any request fired yet" rather than the loop index — spacing
    // must precede every request except the very first.
    ...
    for (let i = 0; i < prefixes.length; i++) {
      ...
      if (useOai) {
        try {
          if (firstRequestMade) await spaceBetweenRequests();
