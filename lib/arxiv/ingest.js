...
export async function harvest(window, deps) {
  ...
  // Apply windowSemantics filter to broad-fetch results before fill-up evaluation.
  // The atom-only path already filters by submittedDate at the API; this is a no-op
  // there but kept for symmetry. OAI's from/until filters on the OAI datestamp
  // (last-updated date), so it surfaces v2-of-old papers re-announced in the
  // window; `submitted-only` drops them by comparing against `published`, which
  // parseOaiRecord anchors on the v1 <date> from arXivRaw.
  const fromDate = window.from; // 'YYYY-MM-DD'
  const untilDate = window.until;
  const passesSemantics = (p) => {
    if (window.windowSemantics === 'submitted-or-updated') return true;
    const pubDate = (p.published ?? '').slice(0, 10); // YYYY-MM-DD
    return pubDate >= fromDate && pubDate <= untilDate;
  };
  ...
  const filledFinal = applyWindowSemanticsToFilled(filledPapers, window);
