...
/**
 * Each step's date range is computed relative to the *original window's `from`*
 * (the broad-fetch lower bound), not the window's `until`. We accept `until` in
 * the signature for caller-side symmetry with `applyFillups`, but don't read it.
 *
 * @returns {Array<{extraDays: number, from: string, until: string}>}
 */
export function planFillupSteps({ from, until: _until, schedule }) {
