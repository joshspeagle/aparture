...
export async function fetchAtom({
  ...
  for (let attempt = 0; attempt <= MAX_RATE_LIMIT_RETRIES; attempt++) {
    ...
    try {
      response = await fetchImpl('/api/fetch-arxiv', {
        ...
        body: JSON.stringify({ query, maxResults: ATOM_MAX_RESULTS, password }),
        signal: fetchSignal,
      });
    } catch (err) {
  ...
  // Single-shot request: hitting the maxResults ceiling means arXiv likely
  // had more matches that were silently dropped. Paging the Atom API without
  // sortBy is unsound (Atom hygiene rules forbid sortBy), so we surface a
  // warning instead of paginating.
  if (entries.length >= ATOM_MAX_RESULTS) {
    statusCallback(
      `⚠ ${subcategory}: hit the ${ATOM_MAX_RESULTS}-result Atom cap — results may be truncated; narrow the date window`
    );
  }
  const papers = [];
