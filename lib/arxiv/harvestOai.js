  ...
  while (true) {
    ...
    for (const recordEl of recordEls) {
      ...
      try {
        ...
        // Only malformed OAI data is skippable; anything else is a
        // programming error that must surface, not a silently dropped record.
        if (!(err instanceof ArxivParseError)) throw err;
        skippedRecords += 1;
