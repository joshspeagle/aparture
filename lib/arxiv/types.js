 ...
 * @property {string} from                            OAI fetch from (YYYY-MM-DD, announcement date inclusive).
 * @property {string} until                           OAI fetch until (YYYY-MM-DD, announcement date inclusive).
 ...
 * @property {number} [targetDaysBack]                Optional. When set (and windowSemantics='submitted-only'),
 *   the orchestrator anchors on the most recent v1 day with content in [from, until]
 *   and keeps only papers with v1 in [anchor - (targetDaysBack-1), anchor]. Without this,
 *   papers are filtered by v1 in [from, until] directly.
 */
