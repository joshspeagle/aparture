...
`reports/` (historical analysis outputs) and `temp/` (Playwright profiles + cached PDFs) are runtime state, gitignored but pinned to the repo root because API routes resolve them via `process.cwd()`. **Do NOT delete** `temp/playwright-profile/` (arXiv reCAPTCHA bypass state) or `temp/notebooklm-profile/` (Google session cookies) unless you're willing to re-authenticate. Phase 2 migrates both dirs to `~/aparture/{reports,cache}/`.
...
| Layer                   | File                  | Used for                                                                             |
| ----------------------- | --------------------- | ------------------------------------------------------------------------------------ |
| **Tokens**              | `styles/tokens.css`   | Palette colours, typography, spacing — consumed via `var(--aparture-*)`              |
| **Shell layout**        | `styles/shell.css`    | Top-level structural classes (`.shell`, `.shell-sidebar`, `.briefing-surface`, etc.) |
...
**Rules of thumb:** always reference `var(--aparture-*)` for colours/typography/spacing (never hardcode palette values); use `className` for shell layout containers, briefing-prose BEM classes, and Lucide icon sizing (`w-*`, `h-*`, `animate-spin`); use inline `style={{...}}` for everything else. Mixing `className` + `style` on the same element is fine and common — `className` for structural, `style` for visual.

**Avoid adding new Tailwind classes.** The utility layer is present in `globals.css` but only Lucide icon sizing + one legacy `md:grid-cols-3` responsive utility are retained.

#### Semantic status colours (hardcoded by design)

These hex values recur with consistent semantic meaning. They have no `--aparture-*` token because they are status indicators, not palette colours:
...
When adding a new status colour, document it here rather than introducing a `--aparture-*` token.
...
Internal refactors with no user-visible change, bug fixes for behavior users weren't aware of, test additions, and `docs/superpowers/**` changes (all gitignored).
